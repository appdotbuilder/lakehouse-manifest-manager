...
import { Server, Code, StickyNote, X } from 'lucide-react';
import type { Manifest, UpdateManifestInput, TargetPlatform, KeyTechnology } from '../../../server/src/schema';

interface ManifestEditorProps {
  manifest: Manifest;
  onSubmit: (data: UpdateManifestInput) => Promise<void>;
  onCancel: () => void;
  isLoading?: boolean;
}
...
export function ManifestEditor({ manifest, onSubmit, onCancel, isLoading = false }: ManifestEditorProps) {
  const [formData, setFormData] = useState<UpdateManifestInput>({
    id: manifest.id,
    name: manifest.name,
    content: manifest.content,
    target_platform: manifest.target_platform,
    key_technologies: [...manifest.key_technologies],
    deployment_notes: manifest.deployment_notes,
  ...
  // Update content when platform or technologies change (but not on initial load)
  useEffect(() => {
    if (isInitialLoad) {
    ...
    const currentPlatform = formData.target_platform || manifest.target_platform;
    const currentTechnologies = formData.key_technologies || manifest.key_technologies;
    
    const boilerplate = generateManifestBoilerplate(currentPlatform, currentTechnologies);
    setFormData((prev: UpdateManifestInput) => ({
      ...prev,
      content: boilerplate
    }));
  }, [formData.target_platform, formData.key_technologies, manifest.target_platform, manifest.key_technologies, isInitialLoad]);
  ...
  const validateForm = () => {
    ...
    if (formData.key_technologies && formData.key_technologies.length === 0) {
          ...
          <div className="space-y-2">
            ...
            <Input
              ...
              onChange={(e: React.ChangeEvent<HTMLInputElement>) =>
                setFormData((prev: UpdateManifestInput) => ({ ...prev, name: e.target.value }))
              }
          ...
          <div className="space-y-2">
            ...
            <Textarea
              ...
              onChange={(e: React.ChangeEvent<HTMLTextAreaElement>) =>
                setFormData((prev: UpdateManifestInput) => ({ ...prev, content: e.target.value }))
              }
        ...
        <CardContent className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="space-y-2">
              ...
              <Select
                value={formData.target_platform || manifest.target_platform}
                onValueChange={(value: TargetPlatform) =>
                  setFormData((prev: UpdateManifestInput) => ({ ...prev, target_platform: value }))
                }
              >
                <SelectTrigger>
